    ...
    return new Promise((resolve) => {
      chrome.storage.local.get(
        ...
        (result) => {
          ...
          const cachedItems = result[this.config.cacheKey] as T[] | undefined;
          const timestamp = result[this.cacheTimestampKey] as
            | number
            | undefined;
...
function parseCSV(
  ...
  if (hasHeaders) {
      ...
      if (options?.expectedHeaders && options.expectedHeaders.length > 0) {
          ...
          if (firstNewline !== -1) {
            headers = parseCsvRow(text.substring(0, firstNewline));
