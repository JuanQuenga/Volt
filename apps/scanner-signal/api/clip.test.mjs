...
test("App Clip scanner endpoint is obsolete", () => {
  const response = makeResponse();

  clipHandler(makeRequest({ path: "ocr", query: { session: "abc_123-safe" } }), response);

  assert.equal(response.statusCode, 410);
  assert.equal(response.headers["content-type"], "text/plain; charset=utf-8");
  assert.equal(response.headers["cache-control"], "no-store");
  assert.match(response.body, /obsolete/);
  assert.doesNotMatch(response.body, /app-clip-bundle-id/);
});

test("apple-app-site-association no longer advertises App Clip scanner routes", () => {
  const response = makeResponse();
  ...
  assert.deepEqual(buildAssociationPayload().appclips.apps, []);
});
